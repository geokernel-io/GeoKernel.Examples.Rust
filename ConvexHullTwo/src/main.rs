...
const LEFT: &[[f64; 2]] = &[[-4.5, -1.6], [-3.1, 1.8], [-1.9, -0.5], [-0.5, 1.4], [-0.1, -1.8], [-2.1, -0.9], [-3.5, -2.0], [-4.5, -1.6]];
const RIGHT: &[[f64; 2]] = &[[0.9, -1.4], [2.3, -2.0], [4.2, -0.2], [3.4, 2.3], [1.6, 1.3], [0.4, 2.7], [0.9, -1.4]];

fn calculate(window: &mut ViewerWindow) -> Result<()>
{
    window.viewer().clear_shapes()?;
    let result: Value = serde_json::from_str(&window.viewer().convex_hull_two_polygons_json(LEFT, RIGHT)?)?;
    ...
    window.clear_log()?;
    window.append_log(&format!("<b>ConvexHullTwo</b><pre>{}</pre>", serde_json::to_string_pretty(&result)?))?;
    window.set_status_text(&format!("ConvexHullTwo: {} result rings, {} vertices", parts.len(), parts.iter().map(Vec::len).sum::<usize>()))?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let events = window.add_control_panel(&json!({"title":"ConvexHullTwo","controls":[
...
{
    bootstrap::prepare()?;
    runtime::main("ConvexHullTwo", "", run)
}
