...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    let controls = window.add_control_panel(&json!({"title":"CoordinateTransform","controls":[
    ...
    while window.is_visible()? {
        ...
        while let Ok(event)=events.try_recv() {
            if event.data.event_type==19 {
                ...
                if (-180.0..=180.0).contains(&e.x_min) && (-85.0..=85.0).contains(&e.y_min) {
                    let (x,y)=window.viewer().transform_point(4326,3857,e.x_min,e.y_min)?;
                    window.set_status_text(&format!("EPSG:4326: {:.6}, {:.6} → EPSG:3857: {x:.2}, {y:.2} m",e.x_min,e.y_min))?;
                }
            }
        }
...
{
    bootstrap::prepare()?;
    runtime::main("CoordinateTransform","world_4326/world_4326.shp",run)
}
