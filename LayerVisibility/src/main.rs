...
fn run(window: &mut ViewerWindow, layers: &[Value]) -> Result<()>
{
    ...
    if !window.add_layer_panel(false, true)? {
        return Err("Could not create layer visibility panel".into());
    }
    ...
    window.set_status_text("Select a layer, then use Change Visibility to hide or show it.")?;
...
fn main() -> Result<()>
{
    ...
    for path in &mut paths {
        *path = path.canonicalize().map_err(|error| format!("{}: {error}. Run ./run.ps1 -Example LayerVisibility first.", path.display()))?;
    }
    ...
    let outcome = (|| {
        let mut window = unsafe { ViewerWindow::new(&runtime, "LayerVisibility", 1200, 800)? };
