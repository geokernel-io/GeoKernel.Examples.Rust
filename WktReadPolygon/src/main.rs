...
const DEFAULT: &str = "POLYGON((-123.25 37.15, -122.15 36.95, -121.55 37.65, -122.05 38.35, -123.05 38.15, -123.25 37.15))";

fn read(window: &mut ViewerWindow, input: &str) -> Result<()>
{
    let raw: Value=serde_json::from_str(&window.viewer().read_wkt_polygon_json(input,input.trim_start().to_uppercase().starts_with("MULTIPOLYGON"))?)?;
    let rings = raw.as_array().ok_or("Expected rings")?.clone();
    let parts: Vec<Vec<[f64;2]>>=rings.iter().map(|r| {
        r.as_array().ok_or("Expected coordinates")?.iter().map(|p| {
            Ok([p["x"].as_f64().ok_or("Missing x")?,p["y"].as_f64().ok_or("Missing y")?])
        }).collect::<Result<_>>()
    }).collect::<Result<_>>()?;
    if parts.is_empty() || parts.iter().any(Vec::is_empty) { return Err("Invalid or empty WKT".into()); }
    let output=window.viewer().write_wkt_polygon(&parts)?;
    let roundtrip: Value=serde_json::from_str(&window.viewer().read_wkt_polygon_json(&output,false)?)?;
    if roundtrip.as_array().is_none_or(Vec::is_empty) { return Err("Roundtrip returned empty geometry".into()); }
    ...
    window.viewer().add_polygon_parts_shape(&projected,&json!({"fillColor":"#F9C74F","fillOpacity":140,"lineColor":"#D95D39","lineWidth":2}).to_string())?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let controls = window.add_control_panel(&json!({"title":"WktReadPolygon","controls":[
...
{
    bootstrap::prepare()?;
    runtime::main("WktReadPolygon","",run)
}
