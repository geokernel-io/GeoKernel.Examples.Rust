...
fn click(window: &mut ViewerWindow, x: f64, y: f64) -> Result<()>
{
    ...
    report(window,&json!({"screenX":x,"screenY":y,"worldX":wx,"worldY":wy}).to_string())?;
    Ok(())
}
...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("MapClickedSignal")?;
    let controls = window.add_control_panel(&json!({"title":"MapClickedSignal","controls":[
        {"id":1,"type":"button","text":"MapClickedSignal"},
...
{
    bootstrap::prepare()?;
    runtime::main("MapClickedSignal","world_4326/world_4326.shp",run)
}
