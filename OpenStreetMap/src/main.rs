...
fn apply(window: &mut ViewerWindow, settings: &Settings) -> Result<()>
{
    ...
    let cache = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../data/xyz_cache/OpenStreetMap");
    std::fs::create_dir_all(&cache)?;
    window.viewer().clear_layers()?;
    if window.viewer().add_open_street_map_layer(settings.cache)? < 0 { return Err("Could not add OSM".into()); }
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let controls=window.add_control_panel(&json!({"title":"OpenStreetMap","controls":[
...
{
    bootstrap::prepare()?;
    runtime::main("OpenStreetMap","",run)
}
