...
{
    editing::run(window, path, "InsertVertex")
}

fn main() -> Result<()>
{
    bootstrap::prepare()?;
    runtime::main("InsertVertex", "world_4326/world_4326.shp", run)
}
...
mod editing
{
    ...
    fn reset(window: &mut ViewerWindow, state: &mut State) -> Result<()>
    {
        ...
        state.serial = 0;
        state.vertices = POLYGON[..POLYGON.len() - 1].to_vec();
        state.part = 0;
        state.vertex = 2;
        state.message.clear();
    ...
    fn command(window: &mut ViewerWindow, state: &mut State, id: i32) -> Result<()>
    {
        ...
        match id {
            ...
            5 => {
                reset(window, state)?;
                window.set_control_value(103, state.vertex as f64, "")?;
            }
            ...
            18 => {
                let hit = selected(window)?;
                require(hit["layerIndex"].as_i64() == Some(i as i64), "Select the editable polygon")?;
                require(state.part == 0, "Choose part 0")?;
                let index = usize::try_from(state.vertex)?;
                let point = insertion_point(&state.vertices, index)?;
                require(
                    window.viewer().insert_selected_feature_vertex_in_edit_layer(state.part, state.vertex, point[0], point[1])?,
                    "Cannot insert vertex into the selected polygon",
                )?;
                state.vertices.insert(index, point);
                state.vertex += 1;
                window.set_control_value(103, state.vertex as f64, "")?;
                state.message = format!("Vertex inserted | Vertices: {} | Point: {:.3}, {:.3}", state.vertices.len(), point[0], point[1]);
            }
            19 => require(
    ...
    fn insertion_point(vertices: &[[f64; 2]], index: usize) -> Result<[f64; 2]>
    {
        if vertices.len() < 2 || index == 0 || index > vertices.len() {
            return Err(format!("Choose a vertex index between 1 and {}", vertices.len()).into());
        }
        let a = vertices[index - 1];
        let b = vertices[index % vertices.len()];
        let dx = b[0] - a[0];
        let dy = b[1] - a[1];
        Ok([(a[0] + b[0]) * 0.5 - dy * 0.22, (a[1] + b[1]) * 0.5 + dx * 0.22])
    }

    #[test]
    fn repeated_insertions_produce_distinct_vertices()
    {
        let mut vertices = POLYGON[..POLYGON.len() - 1].to_vec();
        for index in 2..14 {
            let point = insertion_point(&vertices, index).unwrap();
            assert!(point.iter().all(|coordinate| coordinate.is_finite()));
            assert!(!vertices.contains(&point));
            vertices.insert(index, point);
        }
        assert_eq!(vertices.len(), 18);
        assert!(insertion_point(&vertices, vertices.len()).is_ok());
        assert!(insertion_point(&vertices, 0).is_err());
        assert!(insertion_point(&vertices, vertices.len() + 1).is_err());
    }
    ...
    fn controls(name: &str) -> Vec<Value>
    {
        ...
        if matches!(name, "InsertVertex" | "DeleteVertex") {
            result.extend([
                ...
                json!({"id":103,"type":"number","label":"Insert before vertex","minimum":1,"maximum":i32::MAX,"decimals":0,"value":2}),
            ]);
        }
