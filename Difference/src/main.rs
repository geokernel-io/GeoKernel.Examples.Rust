...
fn calculate(window: &mut ViewerWindow) -> Result<()>
{
    ...
    let result: Value = serde_json::from_str(&window.viewer().difference_polygons_json(LEFT, RIGHT)?)?;
    ...
    window.clear_log()?;
    window.append_log(&format!("<b>Difference</b><pre>{}</pre>", serde_json::to_string_pretty(&result)?))?;
    window.set_status_text(&format!("Difference: {} result rings, {} vertices", parts.len(), parts.iter().map(Vec::len).sum::<usize>()))?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let events = window.add_control_panel(&json!({"title":"Difference","controls":[
