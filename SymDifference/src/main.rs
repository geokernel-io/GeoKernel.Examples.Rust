...
const LEFT: &[[f64; 2]] = &[[-4.4, -1.8], [1.2, -1.8], [1.2, 2.2], [-4.4, 2.2], [-4.4, -1.8]];
const RIGHT: &[[f64; 2]] = &[[-0.2, 3.0], [0.6, 1.2], [3.2, 1.2], [1.1, -0.1], [2.0, -2.0], [-0.2, -0.8], [-2.4, -2.0], [-1.5, -0.1], [-3.6, 1.2], [-1.0, 1.2], [-0.2, 3.0]];

fn calculate(window: &mut ViewerWindow) -> Result<()>
{
    window.viewer().clear_shapes()?;
    let result: Value = serde_json::from_str(&window.viewer().symmetrical_difference_polygons_json(LEFT, RIGHT)?)?;
    ...
    window.viewer().add_polygon_shape(RIGHT, &json!({"fillColor":"#CDE7D8","fillOpacity":100,"lineColor":"#2D6A4F","lineWidth":2}).to_string())?;
    ...
    window.clear_log()?;
    window.append_log(&format!("<b>SymDifference</b><pre>{}</pre>", serde_json::to_string_pretty(&result)?))?;
    window.set_status_text(&format!("SymDifference: {} result rings, {} vertices", parts.len(), parts.iter().map(Vec::len).sum::<usize>()))?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let events = window.add_control_panel(&json!({"title":"SymDifference","controls":[
...
{
    bootstrap::prepare()?;
    runtime::main("SymDifference", "", run)
}
