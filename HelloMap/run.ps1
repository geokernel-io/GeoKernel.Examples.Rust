...
$dataFolder = Join-Path $PSScriptRoot '../data/world_4326'
$Data = Join-Path $dataFolder 'world_4326.shp'

if (!(Test-Path -LiteralPath $Data)) {
    New-Item -ItemType Directory -Force -Path $dataFolder | Out-Null
    $archive = Join-Path $PSScriptRoot '../data/world_4326.zip'
    Write-Host 'Downloading GeoKernel sample: world_4326...'
    Invoke-WebRequest -Uri 'https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/world_4326.zip' -OutFile $archive
    Expand-Archive -LiteralPath $archive -DestinationPath $dataFolder -Force
}

foreach ($extension in '.shp', '.shx', '.dbf') {
    $part = [System.IO.Path]::ChangeExtension($Data, $extension)
