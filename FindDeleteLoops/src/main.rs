...
fn calculate(window: &mut ViewerWindow) -> Result<()>
{
    ...
    let inputs=vec![vec![[-5.0,-1.7], [-1.7,-1.7], [-1.7,1.6], [-5.0,1.6], [-5.0,-1.7]],vec![[0.4,-1.7], [4.5,1.6], [0.4,1.6], [4.5,-1.7], [0.4,-1.7]]];
    let result: Value=serde_json::from_str(&window.viewer().find_and_delete_loops_json(&inputs)?)?;
    ...
    window.clear_log()?;
    window.append_log(&format!("<b>FindDeleteLoops</b><pre>{}</pre>", serde_json::to_string_pretty(&result)?))?;
    window.set_status_text(&format!("FindDeleteLoops: {} result rings, {} vertices", parts.len(), parts.iter().map(Vec::len).sum::<usize>()))?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let events = window.add_control_panel(&json!({"title":"FindDeleteLoops","controls":[
...
{
    bootstrap::prepare()?;
    runtime::main("FindDeleteLoops", "", run)
}
