...
{
    let text = window.viewer().hit_test_top_feature_at_json(x,y,6)?;
    report(window,&text)?;
    Ok(())
}
...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("FeatureAttributes")?;
    let controls = window.add_control_panel(&json!({"title":"FeatureAttributes","controls":[
        {"id":1,"type":"button","text":"FeatureAttributes"},
...
{
    bootstrap::prepare()?;
    runtime::main("FeatureAttributes","world_4326/world_4326.shp",run)
}
