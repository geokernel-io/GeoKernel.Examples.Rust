...
fn calculate(window: &mut ViewerWindow) -> Result<()>
{
    ...
    let inputs=vec![vec![[-4.8,-1.4], [-0.8,-1.4], [-0.8,1.8], [-4.8,1.8], [-4.8,-1.4]],vec![[-2.6,-2.3], [1.2,-2.3], [1.2,0.6], [-2.6,0.6], [-2.6,-2.3]],vec![[0.3,2.8], [0.9,1.1], [2.8,1.1], [1.3,0.1], [2.1,-1.6], [0.3,-0.6], [-1.5,-1.6], [-0.7,0.1], [-2.2,1.1], [-0.3,1.1], [0.3,2.8]],vec![[1.5,-0.2], [4.6,-0.2], [4.6,2.0], [1.5,2.0], [1.5,-0.2]],vec![[2.0,-2.4], [4.8,-1.2], [3.3,0.7], [2.0,-2.4]]];
    ...
    window.clear_log()?;
    window.append_log(&format!("<b>UnionOnList</b><pre>{}</pre>", serde_json::to_string_pretty(&result)?))?;
    window.set_status_text(&format!("UnionOnList: {} result rings, {} vertices", parts.len(), parts.iter().map(Vec::len).sum::<usize>()))?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let events = window.add_control_panel(&json!({"title":"UnionOnList","controls":[
