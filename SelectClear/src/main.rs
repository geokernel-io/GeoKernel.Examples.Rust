...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("SelectClear")?;
    let controls = window.add_control_panel(&json!({"title":"SelectClear","controls":[
...
{
    bootstrap::prepare()?;
    runtime::main("SelectClear","world_4326/world_4326.shp",run)
}
