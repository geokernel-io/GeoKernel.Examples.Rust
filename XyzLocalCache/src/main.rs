...
fn apply(window: &mut ViewerWindow, settings: &Settings) -> Result<()>
{
    ...
    let cache = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../data/xyz_cache/XyzLocalCache");
    std::fs::create_dir_all(&cache)?;
    window.viewer().clear_layers()?;
    if window.viewer().add_xyz_layer("XyzLocalCache",&settings.url,settings.min,settings.max,256,"",settings.cache,&cache.to_string_lossy())? < 0 { return Err("Could not add XYZ layer".into()); }
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let controls=window.add_control_panel(&json!({"title":"XyzLocalCache","controls":[
        {"id":4,"type":"combo","label":"Local cache","options":["Disabled","Enabled"],"value":1},
...
{
    bootstrap::prepare()?;
    runtime::main("XyzLocalCache","",run)
}
