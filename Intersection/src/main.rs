...
const LEFT: &[[f64; 2]] = &[[-4.2, -1.7], [1.0, -1.7], [1.0, 2.1], [-4.2, 2.1], [-4.2, -1.7]];
...
fn calculate(window: &mut ViewerWindow) -> Result<()>
{
    ...
    let result: Value = serde_json::from_str(&window.viewer().intersection_polygons_json(LEFT, RIGHT)?)?;
    ...
    window.clear_log()?;
    window.append_log(&format!("<b>Intersection</b><pre>{}</pre>", serde_json::to_string_pretty(&result)?))?;
    window.set_status_text(&format!("Intersection: {} result rings, {} vertices", parts.len(), parts.iter().map(Vec::len).sum::<usize>()))?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let events = window.add_control_panel(&json!({"title":"Intersection","controls":[
