...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("InfoTool")?;
    let controls = window.add_control_panel(&json!({"title":"InfoTool","controls":[
        {"id":1,"type":"button","text":"InfoTool"},
...
{
    bootstrap::prepare()?;
    runtime::main("InfoTool","world_4326/world_4326.shp",run)
}
