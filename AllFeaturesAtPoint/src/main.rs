...
{
    let text = window.viewer().hit_test_features_at_json(x,y,6)?;
    report(window,&text)?;
    Ok(())
}
...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("AllFeaturesAtPoint")?;
    let controls = window.add_control_panel(&json!({"title":"AllFeaturesAtPoint","controls":[
        {"id":1,"type":"button","text":"AllFeaturesAtPoint"},
...
{
    bootstrap::prepare()?;
    runtime::main("AllFeaturesAtPoint","world_4326/world_4326.shp",run)
}
