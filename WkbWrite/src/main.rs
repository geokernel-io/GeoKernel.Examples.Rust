...
{
    let bytes=window.viewer().write_layer_last_shape_wkb(0)?;
    let decoded:serde_json::Value=serde_json::from_str(&window.viewer().read_wkb_geometry_json(&bytes)?)?;
    if decoded["parts"].as_array().is_none_or(Vec::is_empty) { return Err("WKB roundtrip failed".into()); }
...
fn run(window:&mut ViewerWindow,_path:&str)->Result<()>
{
    ...
    window.add_log_panel("WKB output")?;
    let controls=window.add_control_panel(&json!({"title":"WkbWrite","controls":[
        {"id":1,"type":"combo","label":"Geometry","options":["Point","Polyline","Polygon"],"value":"Point"},
        {"id":2,"type":"button","text":"Clear"},
        {"id":3,"type":"button","text":"Write WKB"}
    ]}).to_string())?;
