...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("ClickHitTest")?;
    let controls = window.add_control_panel(&json!({"title":"ClickHitTest","controls":[
        {"id":1,"type":"button","text":"ClickHitTest"},
...
{
    bootstrap::prepare()?;
    runtime::main("ClickHitTest","world_4326/world_4326.shp",run)
}
