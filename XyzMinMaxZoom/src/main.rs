...
fn apply(window: &mut ViewerWindow, settings: &Settings) -> Result<()>
{
    ...
    let cache = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../data/xyz_cache/XyzMinMaxZoom");
    std::fs::create_dir_all(&cache)?;
    window.viewer().clear_layers()?;
    if window.viewer().add_xyz_layer("XyzMinMaxZoom",&settings.url,settings.min,settings.max,256,"",settings.cache,&cache.to_string_lossy())? < 0 { return Err("Could not add XYZ layer".into()); }
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let controls=window.add_control_panel(&json!({"title":"XyzMinMaxZoom","controls":[
        {"id":2,"type":"number","label":"Min zoom","minimum":0,"maximum":21,"value":0},{"id":3,"type":"number","label":"Max zoom","minimum":0,"maximum":21,"value":19},
...
{
    bootstrap::prepare()?;
    runtime::main("XyzMinMaxZoom","",run)
}
