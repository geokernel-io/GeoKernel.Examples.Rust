...
// This file belongs to this example; no other example's source is required.
pub fn prepare() -> Result<(), Box<dyn Error>>
{
    ...
    // Sequential range reads avoid incomplete COG blocks from concurrent HTTP requests.
    // Apply these to both Windows and the C runtime before GDAL is loaded.
    set_environment("GDAL_HTTP_MULTIRANGE", "SERIAL")?;
    set_environment("GDAL_HTTP_CONNECTTIMEOUT", "5")?;
    set_environment("GDAL_HTTP_TIMEOUT", "60")?;
    set_environment("GDAL_HTTP_MAX_RETRY", "0")?;
