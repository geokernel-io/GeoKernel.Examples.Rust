...
const SOURCE: &[[f64; 2]] = &[[-3.6, -1.7], [-1.5, -2.2], [0.1, -1.1], [2.9, -1.5], [3.6, 0.6], [1.1, 2.1], [-0.9, 1.2], [-3.0, 1.8], [-4.0, 0.0], [-3.6, -1.7]];
...
fn render(window: &mut ViewerWindow, distance: f64) -> Result<()>
{
    ...
    if !window.viewer().add_polygon_buffer_shape(SOURCE, distance, 18, &buffer_style)? { return Err("Buffer operation failed".into()); }
    window.viewer().add_polygon_shape(SOURCE, &source_style)?;
    window.set_status_text(&format!("BufferPolygon — buffer distance: {distance:.2} map units"))?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let events = window.add_control_panel(&json!({"title":"BufferPolygon","controls":[
...
{
    bootstrap::prepare()?;
    runtime::main("BufferPolygon", "", run)
}
