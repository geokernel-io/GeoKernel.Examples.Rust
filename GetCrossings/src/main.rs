...
const LEFT: &[[f64;2]] = &[[-6.0, -2.2], [-4.2, 1.6], [-2.0, -0.5], [0.2, 2.1], [2.4, -0.7], [5.8, 2.2]];
const RIGHT: &[[f64;2]] = &[[-6.2, 1.9], [-3.8, -1.6], [-1.4, 1.5], [1.2, -1.9], [3.2, 1.3], [5.8, -1.2]];
...
fn calculate(window:&mut ViewerWindow,tolerance:f64)->Result<()>
{
    ...
    window.viewer().add_polyline_shape(LEFT,&style)?;
    window.viewer().add_polyline_shape(RIGHT,&json!({"lineColor":"#D95D39","lineWidth":3}).to_string())?;
    let value:Value=serde_json::from_str(&window.viewer().get_polyline_crossings_json(LEFT,RIGHT)?)?;
    let crossings=points(&value)?;
    if crossings.is_empty() { return Err("Expected line crossings".into()); }
    for p in crossings { window.viewer().add_point_shape(p[0],p[1],&json!({"pointColor":"#C1121F","pointSize":12}).to_string())?; }
    window.clear_log()?;
...
fn run(window:&mut ViewerWindow,_path:&str)->Result<()>
{
    ...
    window.add_log_panel("GetCrossings result")?;
    let events=window.add_control_panel(&json!({"title":"GetCrossings","controls":[
