...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    let controls = window.add_control_panel(&json!({"title":"GeoTiffLoad", "controls":[
    ...
    ]}).to_string())?;
    {
        ...
        viewer.set_layer_name(0, "GeoTiffLoad")?;
...
{
    bootstrap::prepare()?;
    runtime::main("GeoTiffLoad", "world_8km_tif/world_8km.tif", run)
}
