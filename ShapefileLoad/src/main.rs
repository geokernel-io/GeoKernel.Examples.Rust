...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    let controls = window.add_control_panel(&json!({"title":"ShapefileLoad", "controls":[
    ...
    ]}).to_string())?;
    {
        ...
        viewer.set_layer_name(0, "ShapefileLoad")?;
...
{
    bootstrap::prepare()?;
    runtime::main("ShapefileLoad", "world_4326/world_4326.shp", run)
}
