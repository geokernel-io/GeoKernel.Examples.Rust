...
$Example = 'DuckDbGeoParquetAnalytics'
if (!$PrepareOnly) {
...
function Get-SampleFile([string]$Folder, [string]$File) {
    ...
    # Archives may include an extra top-level directory. Keep the sample path stable.
    $destination = Join-Path $dataFolder $File
    if ($existing.FullName -ne [System.IO.Path]::GetFullPath($destination)) {
        Copy-Item -LiteralPath $existing.FullName -Destination $destination -Force
    }
    return $destination
}

$Data = Get-SampleFile 'stockholm_data' 'stockholm_buildings.parquet'
