...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    let controls = window.add_control_panel(&json!({"title":"GeoPackageLoad", "controls":[
    ...
    ]}).to_string())?;
    {
        ...
        viewer.set_layer_name(0, "GeoPackageLoad")?;
...
{
    bootstrap::prepare()?;
    runtime::main("GeoPackageLoad", "europe_detailed/europe_detailed.gpkg", run)
}
