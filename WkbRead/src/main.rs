...
const PRESETS: &[&str] = &["010100000050FC1873D79A5EC0D0D556EC2FE34240","01020000000400000050FC1873D79A5EC0D0D556EC2FE34240789CA223B9785EC0ECC039234AAB42401DC9E53FA45F5EC043AD69DE714A434041F163CC5D2F5EC0D26F5F07CED14240","010300000001000000060000000000000000D05EC033333333339342409A99999999895EC09A999999997942403333333333635EC03333333333D342403333333333835EC0CDCCCCCCCC2C43403333333333C35EC033333333331343400000000000D05EC03333333333934240",""];

fn read(window:&mut ViewerWindow,input:&str)->Result<()>
{
    let compact:String=input.chars().filter(|c|!c.is_whitespace()).collect();
    if !compact.is_ascii() || !compact.len().is_multiple_of(2) { return Err("Invalid hex input".into()); }
    let bytes:Vec<u8>=(0..compact.len()).step_by(2).map(|i|u8::from_str_radix(&compact[i..i+2],16)).collect::<std::result::Result<_,_>>()?;
...
fn run(window:&mut ViewerWindow,_path:&str)->Result<()>
{
    ...
    let controls=window.add_control_panel(&json!({"title":"WkbRead","controls":[
        {"id":1,"type":"combo","label":"Preset","options":["Point","LineString","Polygon","MultiPolygon"],"value":"Point"},
        {"id":2,"type":"text","label":"WKB hex","value":PRESETS[0]},
        {"id":3,"type":"button","text":"Read WKB hex"}
    ]}).to_string())?;
