...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("SelectionBoxSignal")?;
    let controls = window.add_control_panel(&json!({"title":"SelectionBoxSignal","controls":[
        {"id":1,"type":"button","text":"Box Select"},
    ...
    let mut signals = Vec::new();
    {
        ...
        viewer.use_tool(ViewerTool::Select);
    }
    runtime::show(window)?;
    ...
    while window.is_visible()? {
        ...
        while let Ok(control) = controls.try_recv() {
            match control.id {
                1 => window.viewer().use_tool(ViewerTool::Select),
                2 => window.viewer().use_tool(ViewerTool::Pan),
                3 => { window.viewer().clear_selected_features()?; report(window,"[]", &signals)?; }
        ...
        while let Ok(event) = events.try_recv() {
            if event.data.event_type == 18 {
                let e = event.data.extent;
                let r = event.data.screen_rectangle;
                let mode = if event.data.int_value & 0x04000000 != 0 {
                    2 // Ctrl: toggle.
                } else if event.data.int_value & 0x02000000 != 0 {
                    1 // Shift: add.
                } else {
                    0 // Replace.
                };
                let count = window.viewer().select_features_in_screen_rect(
                    r.left, r.top, r.right - r.left + 1, r.bottom - r.top + 1, mode,
                )?;
                signals.push(format!("mapSelectionBoxFinished: screen ({}, {}) to ({}, {}); world ({:.4}, {:.4}) to ({:.4}, {:.4}); mode {}; selected {}",
                    r.left, r.top, r.right, r.bottom, e.x_min, e.y_min, e.x_max, e.y_max, mode, count));
                window.set_status_text(&format!("{count} selected feature(s)"))?;
            }
            if event.data.event_type == 11 || event.data.event_type == 18 {
                let text = window.viewer().get_selected_features_json()?;
                report(window, &text, &signals)?;
            }
        }
...
{
    bootstrap::prepare()?;
    runtime::main("SelectionBoxSignal","world_4326/world_4326.shp",run)
}
