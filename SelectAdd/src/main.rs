...
{
    window.viewer().add_top_feature_to_selection_at(x,y,6)?;
...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("SelectAdd")?;
    let controls = window.add_control_panel(&json!({"title":"SelectAdd","controls":[
        {"id":1,"type":"button","text":"SelectAdd"},
...
{
    bootstrap::prepare()?;
    runtime::main("SelectAdd","world_4326/world_4326.shp",run)
}
