...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("BoxSelect")?;
    let controls = window.add_control_panel(&json!({"title":"BoxSelect","controls":[
    ...
    while window.is_visible()? {
        ...
        while let Ok(event) = events.try_recv() {
            if event.data.event_type == 18 {
                let rect = event.data.screen_rectangle;
                // Qt keyboard modifiers: Ctrl toggles hits; Shift adds to the selection.
...
{
    bootstrap::prepare()?;
    runtime::main("BoxSelect","world_4326/world_4326.shp",run)
}
