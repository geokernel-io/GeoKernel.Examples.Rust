...
const LEFT: &[[f64; 2]] = &[[-4.2, -1.7], [0.8, -1.7], [0.8, 2.2], [-4.2, 2.2], [-4.2, -1.7]];
const RIGHT: &[[f64; 2]] = &[[1.0, 3.0], [1.7, 1.2], [3.7, 1.2], [2.1, 0.1], [2.8, -1.8], [1.0, -0.7], [-0.8, -1.8], [-0.1, 0.1], [-1.7, 1.2], [0.3, 1.2], [1.0, 3.0]];

fn calculate(window: &mut ViewerWindow) -> Result<()>
{
    window.viewer().clear_shapes()?;
    let result: Value = serde_json::from_str(&window.viewer().union_polygons_json(LEFT, RIGHT)?)?;
    ...
    window.clear_log()?;
    window.append_log(&format!("<b>Union</b><pre>{}</pre>", serde_json::to_string_pretty(&result)?))?;
    window.set_status_text(&format!("Union: {} result rings, {} vertices", parts.len(), parts.iter().map(Vec::len).sum::<usize>()))?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let events = window.add_control_panel(&json!({"title":"Union","controls":[
