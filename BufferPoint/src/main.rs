...
fn render(window: &mut ViewerWindow, distance: f64) -> Result<()>
{
    ...
    window.set_status_text(&format!("BufferPoint — buffer distance: {distance:.2} map units"))?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let events = window.add_control_panel(&json!({"title":"BufferPoint","controls":[
        {"id":1,"type":"number","label":"Buffer distance","minimum":0.05,"maximum":5,"step":0.05,"value":2.0},
