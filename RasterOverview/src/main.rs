...
fn load(window:&mut ViewerWindow,path:&str,mode:i32)->Result<()>
{
    ...
    let options=json!({"prepareRasterOverviews":mode==1,"rasterOverviewMinimumPixels":if mode==1 {0_i64} else {i64::MAX},"rasterOverviewResampling":"AVERAGE"});
...
{
    if clear {
        window.viewer().get_raster_tile_cache_diagnostics_json(0,false,true)?;
        window.viewer().invalidate_render_cache(true,true)?;
        window.viewer().refresh_layers()?;
    }
    let value:Value=serde_json::from_str(&window.viewer().get_raster_overview_diagnostics_json(0,benchmark)?)?;
    ...
    window.set_status_text(&format!("Overview levels: {} | Run Benchmark to compare zoomed-out read times",
        value["overviews"].as_array().map_or(0,Vec::len)))?;
    Ok(())
}
...
fn run(window:&mut ViewerWindow,path:&str)->Result<()>
{
    ...
    let controls=window.add_control_panel(&json!({"title":"RasterOverview","controls":[
        {"id":1,"type":"combo","label":"Mode","options":["Without overview","With overview"],"value":"Without overview"},
    ...
    while window.is_visible()? {
        ...
        while let Ok(event)=controls.try_recv() {
            match event.id {
                1=>{
                    mode=if event.text=="With overview" {1} else {0};
                    load(window,path,mode)?;
                    diagnostics(window,false,false)?;
                },
...
{
    bootstrap::prepare()?;
    runtime::main("RasterOverview","world_8km_tif/world_8km.tif",run)
}
