...
const LEFT: &[[f64;2]] = &[[-5.8, -1.8], [-5.4, -0.6], [-4.9, 0.2], [-4.2, 1.0], [-3.5, 1.6], [-2.7, 1.9], [-2.0, 1.5], [-1.2, 2.1], [-0.3, 1.7], [0.5, 2.0], [1.4, 1.2], [2.2, 1.4], [3.0, 0.6], [3.8, 0.9], [4.7, 0.1], [5.2, -0.9], [4.2, -1.4], [3.1, -1.1], [2.1, -1.8], [1.1, -1.3], [0.1, -2.0], [-0.9, -1.5], [-1.9, -2.1], [-2.8, -1.5], [-3.8, -1.9], [-4.7, -1.2], [-5.8, -1.8]];
...
fn calculate(window:&mut ViewerWindow,tolerance:f64)->Result<()>
{
    ...
    let value:Value=serde_json::from_str(&window.viewer().simplify_polygon_ring_json(LEFT,tolerance)?)?;
    let simplified=points(&value)?;
    if simplified.len()<4 || simplified.len()>LEFT.len() { return Err("Invalid simplified polygon".into()); }
    window.viewer().add_polygon_shape(&simplified,&json!({"fillColor":"#F9C74F","fillOpacity":150,"lineColor":"#D95D39","lineWidth":3}).to_string())?;
    window.set_status_text(&format!("Vertices: {} → {}",LEFT.len(),simplified.len()))?;
    window.clear_log()?;
...
fn run(window:&mut ViewerWindow,_path:&str)->Result<()>
{
    ...
    window.add_log_panel("ShapeSimplify result")?;
    let events=window.add_control_panel(&json!({"title":"ShapeSimplify","controls":[
...
{
    bootstrap::prepare()?;
    runtime::main("ShapeSimplify","",run)
}
