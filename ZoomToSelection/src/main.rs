...
fn click(window: &mut ViewerWindow, x: f64, y: f64) -> Result<()>
{
    window.viewer().select_top_feature_at(x,y,6)?;
...
fn run(window: &mut ViewerWindow, path: &str) -> Result<()>
{
    ...
    window.add_log_panel("ZoomToSelection")?;
    let controls = window.add_control_panel(&json!({"title":"ZoomToSelection","controls":[
        {"id":1,"type":"button","text":"Select"},
        {"id":2,"type":"button","text":"Pan"},
        {"id":3,"type":"button","text":"Clear Selection"},
        {"id":4,"type":"button","text":"Full Extent"}, {"id":5,"type":"button","text":"Zoom To Selection"}
    ]}).to_string())?;
    ...
    while window.is_visible()? {
        ...
        while let Ok(control) = controls.try_recv() {
            match control.id {
                ...
                5 => {
                    let zoomed = window.viewer().zoom_to_selected_features()?;
                    window.set_status_text(if zoomed {
                        "Zoomed to selected features"
                    } else {
                        "Select a feature on the map before zooming"
                    })?;
                }
                _ => {}
            }
        ...
        while let Ok(event) = events.try_recv() {
            if event.data.event_type == 20 && event.data.int_value == 2 && event.data.int_value2 == 1 {
                click(window,event.data.screen_rectangle.left as f64,event.data.screen_rectangle.top as f64)?;
            }
...
{
    bootstrap::prepare()?;
    runtime::main("ZoomToSelection","world_4326/world_4326.shp",run)
}
