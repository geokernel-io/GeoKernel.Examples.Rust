...
const DEFAULT: &str = "LINESTRING(-122.4194 37.7749, -121.8863 37.3382, -121.4944 38.5816, -120.7401 37.6391)";

fn read(window: &mut ViewerWindow, input: &str) -> Result<()>
{
    let raw: Value=serde_json::from_str(&window.viewer().read_wkt_line_string_json(input)?)?;
    let rings = [raw];
    ...
    let output=window.viewer().write_wkt_line_string(&parts[0])?;
    let roundtrip: Value=serde_json::from_str(&window.viewer().read_wkt_line_string_json(&output)?)?;
    ...
    window.viewer().add_polyline_shape(&projected[0],&json!({"lineColor":"#D95D39","lineWidth":3}).to_string())?;
...
fn run(window: &mut ViewerWindow, _path: &str) -> Result<()>
{
    ...
    let controls = window.add_control_panel(&json!({"title":"WktReadPolyline","controls":[
...
{
    bootstrap::prepare()?;
    runtime::main("WktReadPolyline","",run)
}
